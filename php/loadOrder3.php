...
$uid=$_POST['uid'];
$sql="select orders.oid,card.name as cname,orders.lowprice,orders.num,orders.now_uid as buyer,orders.time,user.name,orders.now_price as price,orders.uid as seller ,orders.oid,orders.cid from card,orders,user where card.cid=orders.cid and user.uid=orders.now_uid and orders.status='ing' and orders.uid=$uid ORDER BY orders.time ASC,TIME DESC";
